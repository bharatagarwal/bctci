...
def valid_adjacency_list(graph):
    ...
    for node, neighbors in enumerate(graph):
        ...
        # invalid node check
        if any(val < 0 or val >= V for val in neighbors):
            return False
        ...
        # mutual membership check
        for neighbor in neighbors:
            neighbor_set = set(graph[neighbor])
            if node not in neighbor_set:
                return False

        # parallel node check
        if len(neighbors) != len(neighbors_set):
            return False
