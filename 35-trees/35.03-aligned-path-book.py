...
def longest_path(node):
    ...
    def longest_path_at(node, depth):
        ...
        current_longest = 0
        left_chain = longest_path_at(node.left, depth + 1)
        right_chain = longest_path_at(node.right, depth + 1)

        if is_aligned(node, depth):
            current_longest = 1 + max(left_chain, right_chain)

            # for path, unlike chain, we cross over to the other side, and therefore the condition changes
            result = max(result, 1 + left_chain + right_chain)
