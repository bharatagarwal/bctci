...
def follow_instructions(sequence):
	...
	if sequence[0] == "L":
		return "L" + follow_instructions(sequence[1:])

	if sequence[0] == "R":
		return "R" + follow_instructions(sequence[1:])

	# python slices are more forgiving than direct indexing
	if sequence[0] == "2":
		return follow_instructions(sequence[1:]) + follow_instructions(
			sequence[2:]
		)
