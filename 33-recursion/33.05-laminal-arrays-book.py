...
def max_sum(laminal):
	def max_sum_recursive(left, right):
		# [left:right] will just be one element
		if right - left == 1:
			# can be return laminal[right] as well
			return laminal[left]

		mid = (left + right) // 2

		op1 = max_sum_recursive(left, mid)
		op2 = max_sum_recursive(mid, right)
		op3 = sum(laminal[left:right])

		return max(op1, op2, op3)

	return max_sum_recursive(0, len(laminal))
